...
export default function ProductDetailPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', paddingTop: '5rem' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '3rem 2rem 6rem' }}>
        ...
        <div className="pd-grid">
          ...
          <div>
            ...
            <h1 style={{ fontSize: 'clamp(1.5rem,3vw,2.1rem)', fontWeight: 900, color: '#111', marginBottom: '1.75rem', lineHeight: 1.2 }}>{product.name}</h1>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '0.5rem', marginBottom: '1.75rem' }}>
              {TIERS.map((t, i) => (
                <div key={i} style={{ textAlign: 'center', padding: '0.8rem 0.25rem', borderRadius: 12, background: i === activeTier ? 'rgba(255,71,87,0.06)' : '#f8f8f8', border: `1.5px solid ${i === activeTier ? 'rgba(255,71,87,0.35)' : 'rgba(0,0,0,0.08)'}`, transition: 'all 0.2s' }}>
                  ...
                  <div style={{ fontSize: '1.05rem', fontWeight: 900, color: i === activeTier ? '#ff4757' : '#111' }}>${(product.basePrice * t.mult).toFixed(2)}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '2.5rem' }}>
              ...
              <Link href={`/sample-order?product=${id}`}
                style={{ width: '100%', padding: '15px', background: '#fff', color: '#111', border: '1.5px solid rgba(0,0,0,0.18)', borderRadius: 50, fontWeight: 700, fontSize: '0.9rem', textDecoration: 'none', letterSpacing: '0.06em', textTransform: 'uppercase', textAlign: 'center', display: 'block', boxSizing: 'border-box' }}
              >
                Order a Sample
              </Link>
            </div>
      ...
      <style>{`
        .pd-grid { display: grid; grid-template-columns: 55% 45%; gap: 3.5rem; align-items: start; }
