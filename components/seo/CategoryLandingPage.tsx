...
const ROOT_SLUGS = new Set(SEO_LANDING_PAGES.map((c) => c.slug))

function relatedHref(slug: string) {
  return ROOT_SLUGS.has(slug) ? `/${slug}` : `/products/${slug}`
}
...
export default function CategoryLandingPage({ cat, breadcrumbHref = '/products', breadcrumbLabel = 'Products' }: Props) {
  return (
      ...
      <section style={{ paddingTop: '100px', paddingBottom: '60px', background: '#f5f5f5', borderBottom: '1px solid #e8e8e8' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 2rem' }}>
          <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
            <Link href={breadcrumbHref} style={{ color: '#999', fontSize: '0.85rem', textDecoration: 'none' }}>{breadcrumbLabel}</Link>
        ...
        <section style={{ maxWidth: 1200, margin: '0 auto', padding: '0 2rem 4rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            {cat.relatedCategories.map((slug) => (
              <Link key={slug} href={relatedHref(slug)} className="cc-related-link">
